...
/* Some cheap overloading of the equals operator. */
const AppEngine& AppEngine::operator=(const AppEngine& other)
{
	if (this == &other)
	{
		return *this;
...
void AppEngine::Initialise(HWND hWnd)
{
	...
	this->dLights.push_back(DirectionalLight(Vector3D(2,0,0)));
...
void AppEngine::Render(void)
{
	...
	this->_model.CalculateAmbientLight(_camera.GetPosition(), this->_amblight); // Calculate ambient light based off the object value here
	this->_model.CalculateLightingDirectional(_camera.GetPosition(),dLights);
	...
	this->_model.CalculateLightingDirectional_Vertex(_camera.GetPosition(),dLights);
